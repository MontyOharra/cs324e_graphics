...
void setup() {
  size(600, 600, P3D);
  dog = new Dog(0, 0, 0);
  dog2 = new Dog(-100, 0, -100);
  player = new Player(100, 50, 0, .5, .01, 1,0, 0);
  player2 = new Player(300, 50, 0, .7, .07, 1,0, 0);
  sky = new Sky();
  smooth();
}
...
void draw() {
  ...
  background(135, 206, 250); // Light blue sky color
  lights();
  
  sky.update();
  sky.display();
  // trees
  tree = new Tree(height/3, width/2, 0);
  tree2 = new Tree(-100, 0, 80);
  tree.display();
  tree2.display();
  //dogs
  dog.render();
